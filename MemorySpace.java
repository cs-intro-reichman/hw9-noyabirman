...
public class MemorySpace {
	...
	public void free(int address) {
		if (address<0) throw new IllegalArgumentException("address can't be less than 0");
		if (allocatedList.getSize()==0) throw new IllegalArgumentException("index must be between 0 and size");
		Node current= allocatedList.getFirst();
		while (current != null) {
			if (current.block.baseAddress == address) break;
			current= current.next;
	...
	public void defrag() {
		...
		while (mainIterator.hasNext()) {
			MemoryBlock MemoryBlock1 = mainIterator.next();
			int baseSearch = MemoryBlock1.baseAddress + MemoryBlock1.length;
			while (secondIterator.hasNext()) {
				MemoryBlock MemoryBlock2 = secondIterator.next();
				if (baseSearch == MemoryBlock2.baseAddress) {
					MemoryBlock1.length += MemoryBlock2.length;
					freeList.remove(MemoryBlock2);
					defrag();
				}
			}
		}
